...
We focus on a subset of our data to perform the analysis. To create this subset we use a paired data criterion. That is, we keep only the data corresponding to patient that have samples of tumor and normal type. The following code performs this filtering:
...
```
This figure reveals substantial differences in sequencing depth between samples, although this differences are not quite homogeneously distributed between normal and tumor samples. 
...
variation separates tumor from normal samples. We can also observe that a few normal samples are separated from the rest, in a more clear separation than that shown in the hierchical
clustering. We may consider discarding these two samples and doing the
